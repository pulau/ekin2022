...
class Rm_kamal1 extends MY_Controller {
    public $data = array();
    ...
    function index()
    {
        $is_permit = $this->aauth->control_no_redirect('rm_kamal1_loket_perm');
        if(!$is_permit) {
        ...
        $this->data['bc_child'] = "RM PKL Kamal 1";
        $perms = "rm_kamal1_loket_perm";
        $comments = "Halaman Loket RM PKL Kamal 1";
        $this->aauth->logit($perms, current_url(), $comments);
    	$this->load->view('rm_kamal1', $this->data);
    }
    
    public function ajax_list_mr(){
        $list = $this->Rm_model->get_mr_list(PKL_KAMAL1);
        $data = array();
        ...
        $output = array(
                    ...
                    "recordsTotal" => $this->Rm_model->count_mr_all(PKL_KAMAL1),
                    "recordsFiltered" => $this->Rm_model->count_mr_filtered(PKL_KAMAL1),
                    "data" => $data,
                    );
    ...
    public function do_insert_mr(){
        ...
        if ($this->form_validation->run() == FALSE)  {
            ...
            $no_mr = $this->Rm_model->generate_mr_number(PKL_KAMAL1);
