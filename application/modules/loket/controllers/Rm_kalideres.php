...
class Rm_kalideres extends MY_Controller {
    public $data = array();
    ...
    function index()
    {
        $is_permit = $this->aauth->control_no_redirect('rm_kalideres_loket_perm');
        if(!$is_permit) {
        ...
        $this->data['bc_child'] = "RM PKC Kalideres";
        $perms = "rm_kalideres_loket_perm";
        $comments = "Halaman Loket RM PKC Kalideres";
        $this->aauth->logit($perms, current_url(), $comments);
    	$this->load->view('rm_kalideres', $this->data);
    }
    
    public function ajax_list_mr(){
        $list = $this->Rm_model->get_mr_list(PKC_KALIDERES);
        $data = array();
        ...
        $output = array(
                    ...
                    "recordsTotal" => $this->Rm_model->count_mr_all(PKC_KALIDERES),
                    "recordsFiltered" => $this->Rm_model->count_mr_filtered(PKC_KALIDERES),
                    "data" => $data,
                    );
    ...
    public function do_insert_mr(){
        ...
        if ($this->form_validation->run() == FALSE)  {
            ...
            $no_mr = $this->Rm_model->generate_mr_number(PKC_KALIDERES);
