...
$(document).ready(function(){
  ...
  //      "scrollX": true,
        ...
        "ajax": {
            "url": ci_baseurl + "loket/rm_semanan2/ajax_list_mr",
            "type": "GET"
        },
    ...
    $('button#btnSaveRM').click( function() {
        jConfirm("Anda yakin akan menyimpan data ini ?","Ok","Cancel", function(r){
            if(r){
                var used_url = ci_baseurl + "loket/rm_semanan2/do_insert_mr";
                $.ajax({
                    url: used_url,
                    type: 'POST',
                    dataType: 'JSON',
    ...
    $('button#btnUpdateRM').click( function() {
        jConfirm("Anda yakin akan mengubah data ini ?","Ok","Cancel", function(r){
            if(r){
                var used_url = ci_baseurl + "loket/rm_semanan2/do_update_mr";
                $.ajax({
                    url: used_url,
                    type: 'POST',
                    dataType: 'JSON',
...
function editPasien(rm_id){
    if(rm_id !==''){		
        $.ajax({
        url: ci_baseurl + "loket/rm_semanan2/ajax_get_rm_by_id",
        type: 'get',
        dataType: 'json',
        data: {rm_id:rm_id},
            success: function(data) {
