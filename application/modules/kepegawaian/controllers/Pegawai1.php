            ...
            if (!$this->upload->do_upload('file_excel')){
                ...
                if(count($values) > 0){
                    foreach($values as $val){
                        $email = !empty($val['G']) ? $val['G'] : 'default@mail.com';
                        $passwd = '12345';
                        $nm_lengkap = $val['B'];
                        $nip = $val['A'];
                        $group = 21;
                        $tempat_lahir = $val['C'];
                        $tgl_lahir = date('Y-m-d',strtotime($val['D']));
                        $jenis_kelamin = $val['E'];
                        $no_tlp = $val['F'];
                        $alamat = $val['H'];
                        $no_ktp = $val['I'];
                        $npwp = $val['J'];
                        $status_pns = $val['K'];
                        $tgl_masuk = date('Y-m-d',strtotime($val['L']));
                        $gelar_depan = $val['M'];
                        $gelar_belakang = $val['N'];
                        $status_kawin = $val['O'];
                        $pendidikan = $val['P'];
                        $rumpun_jabatan = $val['Q'];
                        $status_pajak = $val['R'];
                        $bpjs_kes = $val['S'];
                        $bpjs_jkk_jkm = $val['T'];
                        $bpjs_ijht = $val['U'];
...
//                );
//                
//                $insert_excel = $this->Pegawai_model->insert_excel($data_excel);
                if($this->db->trans_status() === FALSE){
                    $this->db->trans_rollback();
                    $res = array(
                    'csrfTokenName' => $this->security->get_csrf_token_name(),
