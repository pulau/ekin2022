...
class Tunjangan_pegawai extends MY_Controller
{
	public $data = array();
	...
	function index()
	{
		$is_permit = $this->aauth->control_no_redirect('tunjangan_pegawai_ekinerja_perm');
		if (!$is_permit) {
			$this->session->set_flashdata('notification', $this->lang->line('aauth_error_no_access'));
			redirect('no_permission');
		}

		$this->data['bc_parent']	= "Tunjangan Pegawai";
		$this->data['bc_child']		= "Dashboard";
		$perms = 'gaji_pegawai_ekinerja_perm';
		$comments = "Halaman Dashboard Tunjangan Pegawai";
		$this->aauth->logit($perms, current_url(), $comments);
		$this->load->view('tunjangan_pegawai', $this->data);

	}

	public function ajax_list_tunjangan_pegawai(){
        $filter_bln = $this->input->get('filter_bln',TRUE);
        $list = $this->Gaji_pegawai_model->get_tunjangan_pegawai_list($filter_bln);
        $data = array();
        ...
        foreach($list as $tkd){
            ...
            $row[] = $no;
            $row[] = $tkd->nip;
            $row[] = $tkd->nama_pegawai;
            $row[] = $tkd->masakerja." Tahun";
            $row[] = "Rp. ".number_format($tkd->gapok,2,',','.');
            $row[] = "Rp. ".number_format($tkd->capaian_kinerja,2,',','.');
            $row[] = "Rp. ".number_format($tkd->tkd,2,',','.');
            $row[] = "Rp. ".number_format($tkd->bpjs_ket,2,',','.');
            $row[] = "Rp. ".number_format($tkd->bpjs_kes,2,',','.');
            $row[] = "Rp. ".number_format($tkd->total_terima,2,',','.');
            
            $data[] = $row;
        }
        $output = array(
                    "draw" => $this->input->get('draw'),
                    "recordsTotal" => $this->Tunjangan_pegawai_model->get_tunjangan_pegawai_all($filter_bln),
                    "recordsFiltered" => $this->Gaji_pegawai_model->count_tunjangan_pegawai_filtered($filter_bln),
                    "data" => $data,
                    );
