...
class Pengajuan_cuti_model extends CI_Model {
    ...
    public function get_pengajuan_list($username, $filter_bln){
        $this->db->from('v_cutipegawai');
        
        if(!empty($filter_bln)){
            $filter_bln = date('Y-m',strtotime($filter_bln));
            //$this->db->where('tgl_awal BETWEEN "'.date('Y-m-d', strtotime($tgl_awal)).'" AND "'.date('Y-m-d', strtotime($tgl_akhir)).'"');
            $this->db->like('tgl_awal',$filter_bln,'after');
        }
    ...
    public function count_pengajuan_all($username, $filter_bln){
        $this->db->from('v_cutipegawai');
    ...
    public function count_pengajuan_filtered($username, $filter_bln){
        $this->db->from('v_cutipegawai');
    ...
    function get_sisa_cuti($pegawai_id, $tempattugas){
        ...
        $this->db->like('tgl_awal',$current_year,'after');
        $query2 = $this->db->get();
        $res = $query2->row();
        $jml_cuti = $res->jml;
    ...
    function get_jumlah_cuti($tempattugas){
        if($tempattugas == "KECAMATAN"){
            ...
            $res = $query->row();
            $total_cuti = $res->sysconf_value;
        }else if($tempattugas == "SHIFT"){
            $query = $this->db->get_where('p_sysconf', array('sysconf_nama' => 'jatah_cuti_per_tahun_shift'), 1, 0);
            $res = $query->row();
            $total_cuti = $res->sysconf_value;
        }
    ...
    public function list_pegawai($id_pegawai, $bagian_id){
    ...
    public function insert_cuti_detail($data){
        $insert = $this->db->insert('t_cutidetail',$data);
    ...
    public function delete_cuti($id){
        $delete = false;
        $delete_detail = $this->db->delete('t_cutidetail', array('cuti_id' => $id));
        if($delete_detail){
            $delete = $this->db->delete('t_cuti', array('cuti_id' => $id));
        }
